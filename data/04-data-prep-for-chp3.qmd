...
ggsave("./figures/chp3-toronto_CMA_plot.png", width = 10, height = 8, dpi = 300)
```
...
ggsave("./figures/chp3-toronto_popden_NIAs_plot.png", width = 10, height = 8, dpi = 300)
```
...
ggplot() +
  ...
  annotation_north_arrow(location = "tl", which_north = "true", 
                         height = unit(1, "cm"), width = unit(1, "cm"))
ggsave("./figures/chp3-toronto_popden_DBCent_plot.png", width = 10, height = 8, dpi = 300)
```
...
ggplot() +
  ...
  annotation_north_arrow(location = "tl", which_north = "true", 
                         height = unit(1, "cm"), width = unit(1, "cm"))
ggsave("./figures/chp3-parkland_paths_plot.png", width = 10, height = 8, dpi = 300)
```
...
ggsave("./figures/chp3-park_entrance_example_plot.png", width = 10, height = 10, dpi = 300)
```
...
img1 <- readPNG("./figures/intermediate/chp3-scatter_tt_cent_vs_ent_BICYCLE_plot.png")
img2 <- readPNG("./figures/intermediate/chp3-scatter_tt_cent_vs_ent_CAR_plot.png")
img3 <- readPNG("./figures/intermediate/chp3-scatter_tt_cent_vs_ent_WALK_plot.png")
img4 <- readPNG("./figures/intermediate/chp3-scatter_tt_cent_vs_ent_TRANSIT_plot.png")
...
png("./figures/chp3-ent_vs_cent_tt_walk_cycle_scatter.png", width = 10, height = 5, units = "in", res = 300)
grid.draw(ent_vs_cent_tt_walk_cycle_scatter)
dev.off()
```
...
# Save the raster-based grid as a PNG
png("./figures/chp3-ent_vs_cent_tt_transit_car_scatter.png", width = 10, height = 5, units = "in", res = 300)
grid.draw(ent_vs_cent_tt_transit_car_scatter)
dev.off()
```
...
plot_data <- expand.grid(
  ...
  mutate(value = case_when(
      ...
      mode == "Cycling" & park_type == "Parkette" ~ cutoff_func(c_ij, 15), #3
      mode == "Cycling" & park_type == "Small" ~ cutoff_func(c_ij, 15),#5
      mode == "Cycling" & park_type == "Medium" ~ cutoff_func(c_ij, 15),#8
      mode == "Cycling" & park_type == "Large" ~ cutoff_func(c_ij, 15),#15
      mode == "Cycling" & park_type %in% c("City", "Legacy") ~ cutoff_func(c_ij, 15),#30
      mode == "Walking" & park_type == "Parkette" ~ cutoff_func(c_ij, 15),#15
      mode == "Walking" & park_type == "Small" ~ cutoff_func(c_ij, 15),#17
      mode == "Walking" & park_type == "Medium" ~ cutoff_func(c_ij, 15),#25
      mode == "Walking" & park_type %in% c("Large", "City", "Legacy") ~ cutoff_func(c_ij, 15),#30
      TRUE ~ 0)) |>
  ungroup()

plot_data <- plot_data |>
      mutate(
        park_type_2 = case_when(park_type == "Large" | park_type == "City" | park_type == "Legacy" ~ "Large",
                                park_type == "Parkette" | park_type == "Small" | park_type == "Medium" ~ "Small", TRUE ~ NA))
```

```{r}
green_palette <- brewer.pal(n = 9, name = "Greens")[c(9,5)]
...
scale_linetype_manual(name = "",
  ...
  geom_line(data =plot_data, aes(x = c_ij, y = value, color = park_type_2),
            size=1, alpha = 0.8) +
...
ggsave("./figures/chp3-norm_pos_impedance_mode_parktype_plot.png", width = 8, height = 6, dpi = 300)
```
