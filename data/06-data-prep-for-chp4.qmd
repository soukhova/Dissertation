...
## 4. calc B_j and A_i
```{r}
B_j_WALK <- access_WALK |> filter(f != 0) |>
  group_by(P_ID) |>
  summarize(park_a = first(area_ha), 
            B_j_WALK = ifelse(sum(POP2021 * f) == 0, 0, 1 / sum(POP2021 * f)))
  
A_i_WALK <- access_WALK |> filter(f != 0) |> 
  group_by(from_id) |>
  summarize(A_i_WALK = ifelse(sum(area_ha * f) == 0, 0, 1 / sum(area_ha * f)))
```

```{r}
access_WALK |> filter(f != 0) |> group_by(P_ID) |> summarize(park_a = first(area_ha)) |> pull(park_a) |> sum()
```

## 5. calc singly constrained V and M
```{r}
access_WALK <- access_WALK |>  filter(POP2021 !=0) |> 
  left_join(B_j_WALK,
            by = "P_ID") |>
...
#checks 
access_WALK$V_opp_ij |> sum(na.rm=T) #It is 7592.858, because the total reachable 7632.922 ha actually can't be fully allocated, as some origins have 0 population.
...
#checks 
access_WALK$M_pop_ji |> sum(na.rm=T) #should be 2761103 people total
access_WALK$M_tot_ji |> sum() #should be 2761103 people total
...
```{r}
access_WALK_i <- TO_census_21_DB |>
  left_join(access_WALK_i, by=c(`Region Name` = "from_id")) |>
...
for fun
```{r}
orig |> filter(id == "35202845001")
access_allmodes[from_id == "35202845001"]
```
...
breaks_V_i_0 <- quantile(access_WALK_i$V_i_0, probs = seq(0, 1, 0.25), na.rm = TRUE)
breaks_V_tot_i <- quantile(access_WALK_i$V_tot_i, probs = seq(0, 1, 0.25), na.rm = TRUE) 
breaks_V_opp_i <- quantile(access_WALK_i$V_opp_i, probs = seq(0, 1, 0.25), na.rm = TRUE) 

#creating factor quartiles for the bins
access_WALK_i$V_i_0_quart <- cut(access_WALK_i$V_i_0, 
                                 breaks = breaks_V_i_0, 
                                 include.lowest = TRUE, 
                                 labels = c("Q1", "Q2", "Q3", "Q4"))

access_WALK_i$V_tot_i_quart <- cut(access_WALK_i$V_tot_i,
                                   breaks = breaks_V_tot_i, 
                                   include.lowest = TRUE, 
                                   labels = c("Q1", "Q2", "Q3", "Q4"))

access_WALK_i$V_opp_i_quart <- cut(access_WALK_i$V_opp_i,
...
legend_labels_V_tot_i <- paste0(round(breaks_V_tot_i[-length(breaks_V_tot_i)], 4), "–",
  round(breaks_V_tot_i[-1], 4), " (",
  c("0–25th", "25–50th", "50–75th", "75–100th"), ")")

legend_labels_V_opp_i <- paste0(round(breaks_V_opp_i[-length(breaks_V_opp_i)], 4), "–",
  round(breaks_V_opp_i[-1], 4), " (",
  c("0–25th", "25–50th", "50–75th", "75–100th"), ")")
...
V_0_i_access_WALK_plot <- ggplot() +
  ...
  geom_sf(data = TO_census_21_DB |> filter(Population==0) |> 
            mutate(pop_label = "No pop & no access"),
           aes(fill = pop_label), col= NA) + #there is NO population
  geom_sf(data = TO_census_21_DB |> filter(Population!=0) |>
            mutate(pop_label = "DB with pop & no access"), 
          aes(fill = pop_label), col= NA) + #there IS population
  scale_fill_manual(values = c("DB with pop & no access" = "grey80",
                               "No pop & no access" = "grey95"),
                    name = "",
                    guide = guide_legend(order = 2)) +
  new_scale_fill() +
  
  geom_sf(data = access_WALK_i |> filter(!is.na(V_i_0_quart)),
          aes(fill = V_i_0_quart), col = NA) +
  scale_fill_manual(values = setNames(oranges, quartile_levels),
...
V_T_i_access_WALK_plot <- ggplot() +
  ...
  scale_fill_manual(values = c("DB with pop & no access" = "grey80",
                    ...
                    guide = guide_legend(order = 2)) +
  new_scale_fill() +
  geom_sf(data = access_WALK_i |> filter(!is.na(V_tot_i_quart)),
          aes(fill = V_tot_i_quart), col = NA) +
  scale_fill_manual(values = setNames(purples, quartile_levels),
...
V_S_i_access_WALK_plot <- ggplot() +
  ...
  scale_fill_manual(values = c("DB with pop & no access" = "grey80",
                    ...
                    guide = guide_legend(order = 2)) +
  new_scale_fill() +
  geom_sf(data = access_WALK_i |> filter(!is.na(V_opp_i_quart)),
...
save(access_WALK_i_df, file="./intermediate/access_WALK_i_df.rda")
```

Correlation?
```{r}
ggplot(access_WALK_i_df,
       aes(x = V_opp_i, y = V_tot_i)) +
  geom_point() +
  labs(
    x = "V_opp_i",
    y = "V_tot_i") +
...
access_WALK_i_df <- access_WALK_i_df  |>
  mutate(
    x_decile = ntile(V_opp_i, 10),
    y_decile = ntile(V_tot_i, 10)
  )

# Calculate labels for x axis deciles inline
x_deciles <- quantile(access_WALK_i_df$V_opp_i, probs = seq(0, 1, 0.1), na.rm = TRUE)
y_deciles <- quantile(access_WALK_i_df$V_tot_i, probs = seq(0, 1, 0.1), na.rm = TRUE)

# Create labels for axes
x_labels <- paste0(
  round(head(x_deciles, -1), 2), "–", round(tail(x_deciles, -1), 2)
)
y_labels <- paste0(
  round(head(y_deciles, -1), 2), "–", round(tail(y_deciles, -1), 2)
)
...
test_labeled <- test |> left_join(parkland |> st_drop_geometry() |> select(c("P_ID", "park_name")) |> distinct(), by="P_ID") |>
  mutate(diff = V_tot_i - V_opp_i) |>
  filter(diff > 0) |>
  group_by(park_size) |>
  top_n(3, wt = diff) |>
  ungroup()
...
ggplot(test, aes(x = V_opp_i, y = V_tot_i)) +
  ...
  geom_text_repel(data = test_labeled,
    aes(label = park_name), 
    max.overlaps = 10,
    box.padding = 0.3,
    point.padding = 0.1,
    size = 3,
    alpha=0.8) +
  facet_wrap(~ park_size, scales = "free", ncol=3) +
  labs(x = "Singly Constrained",
    y = "Total Constrained") +theme_minimal()

ggsave(file="./figures/chp4-singly_total_by_parksize_scatter.png", width = 10, height = 8, dpi = 300)
```
...
save(access_WALK_i_neighbourhoods, file="./intermediate/access_WALK_i_neighbourhoods_df.rda")
```

Creating labels for the opportunities map legends:
```{r}
#creating factor quartiles for the bins
access_WALK_i_neighbourhoods$V_neigh_0_quart <- cut(access_WALK_i_neighbourhoods$V_neigh_0, 
                                 breaks = breaks_V_neigh_0, 
                                 include.lowest = TRUE, 
                                 labels = c("Q1", "Q2", "Q3", "Q4"))

access_WALK_i_neighbourhoods$V_tot_neigh_quart <- cut(access_WALK_i_neighbourhoods$V_tot_neigh,
                                   breaks = breaks_V_tot_neigh, 
                                   include.lowest = TRUE, 
                                   labels = c("Q1", "Q2", "Q3", "Q4"))

access_WALK_i_neighbourhoods$V_opp_neigh_quart <- cut(access_WALK_i_neighbourhoods$V_opp_neigh,
                                   breaks = breaks_V_opp_neigh, 
                                   include.lowest = TRUE, 
                                   labels = c("Q1", "Q2", "Q3", "Q4"))
```
Create the labels:
...
V_0_neigh_access_WALK_plot <- ggplot() +
  ...
  geom_sf(data = access_WALK_i_neighbourhoods |> filter(!is.na(V_neigh_0_quart)),
          aes(fill = V_neigh_0_quart), col = NA) +
  scale_fill_manual(values = setNames(oranges, quartile_levels),
                    name = "Parkland-ha-≤15-min",
  ...
  geom_sf_pattern(data = neighbourhoods,
                  aes(pattern = factor(CLASSIF9)), fill= NA,
                  pattern_density = 0.15, pattern_spacing = 0.03,
                  color = "grey30", size = 0.1, pattern_alpha = 0.4) +
  ...
  geom_sf(data = TO_boundary, col ="grey30", size = 6, fill = NA) +
  geom_sf(data=parkland_grouped |> mutate(park_label = "Parks"),
          aes(fill=park_label), col=NA, alpha=0.6)+
  scale_fill_manual(values = c("Parks" = "forestgreen"),
                    name = "",
                    guide = guide_legend(order = 3)) +
  coord_sf(xlim = c(toronto_bbox["xmin"], toronto_bbox["xmax"]),
...
V_T_neigh_access_WALK_plot <- ggplot() +
  ...
  geom_sf(data = access_WALK_i_neighbourhoods |> filter(!is.na(V_tot_neigh_quart)),
          aes(fill = V_tot_neigh_quart), col = NA) +
  scale_fill_manual(values = setNames(purples, quartile_levels),
                    name = "Accessible Parkland ha",
...
V_S_neigh_access_WALK_plot <- ggplot() +
  ...
  geom_sf(data = access_WALK_i_neighbourhoods |> filter(!is.na(V_opp_neigh_quart)),
          aes(fill = V_opp_neigh_quart), col = NA) +
  scale_fill_manual(values = setNames(purples, quartile_levels),
                    name = "Accessible Parkland ha",
...
Calc quartile (for mapping) of the opportunities value, and labels. Only for unconstrained:
```{r}
#now finding the capped 0,0.25, 0.5, and 0.75 and 1 percentiles for the caped distribution
...
Creating labels for the constrained values:
```{r}
access_WALK_i_neighbourhoods$vv_tot_neigh_cat <- cut(
  access_WALK_i_neighbourhoods$vv_tot_neigh,
  breaks = c(-Inf, 2, 4, 12, 28, Inf),
  labels = c("0–2", "2–4 (Hula Hoop)", "4–12 (Patio Umbrella)", "12–28 (Bus Shelter)", "28+ (Mid-size Tree Canopy)"),
  right = FALSE,
  levels = c("0–2", "2–4 (Hula Hoop)", "4–12 (Patio Umbrella)", "12–28 (Bus Shelter)", "28+ (Mid-size Tree Canopy)"))

access_WALK_i_neighbourhoods$vv_opp_neigh_cat <- cut(
  access_WALK_i_neighbourhoods$vv_opp_neigh,
  breaks = c(-Inf, 2, 4, 12, 28, Inf),
  labels = c("0–2", "2–4 (Hula Hoop)", "4–12 (Patio Umbrella)", "12–28 (Bus Shelter)", "28+ (Mid-size Tree Canopy)"),
  right = FALSE,
  levels = c("0–2", "2–4 (Hula Hoop)", "4–12 (Patio Umbrella)", "12–28 (Bus Shelter)", "28+ (Mid-size Tree Canopy)"))
...
```{r, eval=FALSE}
yellowgreen <- c("#FFFF33", "#C2E699", "#78C679", "#238443")
redyellowgreen <-  brewer.pal(9, "RdYlGn")[c(1,2,3,7,8)]  
vv_0_neigh_access_WALK_plot <- ggplot() +
  ...
  geom_sf(data=parkland_grouped |> mutate(park_label = "Parks"),
          aes(fill=park_label), col=NA, alpha=0.7)+
  scale_fill_manual(values = c("Parks" = "#004225"),
                    name = "",
                    guide = guide_legend(order = 3)) +
...
vv_T_neigh_access_WALK_plot <- ggplot() +
  geom_sf(data = lake_ontario, fill="lightblue", col=NA, size = 0.3) +
    geom_sf(data = access_WALK_i_neighbourhoods,
          aes(fill = vv_tot_neigh_cat), col = NA) +
  geom_sf_pattern(data = neighbourhoods, aes(pattern = factor(CLASSIF9)), fill= NA,
                  pattern_density = 0.15, pattern_spacing = 0.03, color = "grey30", size = 0.1, pattern_alpha = 0.4) +

  scale_fill_manual(name = "Accessible Parkland\nSqm Per Capita",
                    values =setNames(redyellowgreen,
                                  levels(access_WALK_i_neighbourhoods$vv_tot_neigh_cat)),
                    na.value = "white")+
  scale_pattern_manual(values = c("Neighbourhood Improvement Area" = "stripe",
                                  "Emerging Neighbourhood" = "circle", 
                                  "Not an NIA or Emerging Neighbourhood"= "none"),
...
vv_S_neigh_access_WALK_plot <- ggplot() +
  geom_sf(data = lake_ontario, fill="lightblue", col=NA, size = 0.3) +
      geom_sf(data = access_WALK_i_neighbourhoods,
          aes(fill = vv_opp_neigh_cat), col = NA) +
  geom_sf_pattern(data = neighbourhoods, aes(pattern = factor(CLASSIF9)), fill= NA,
                  pattern_density = 0.15, pattern_spacing = 0.03, color = "grey30", size = 0.1, pattern_alpha = 0.4) +
  scale_pattern_manual(values = c("Neighbourhood Improvement Area" = "stripe",
                                  "Emerging Neighbourhood" = "circle", 
    ...
    scale_fill_manual(name = "Accessible Parkland\nSqm Per Capita",
                    values =setNames(redyellowgreen,
                                  levels(access_WALK_i_neighbourhoods$vv_opp_neigh_cat)),
                    na.value = "white")+
  new_scale_fill() +
  geom_sf(data = TO_boundary, col ="grey30", size = 6, fill = NA) +
  geom_sf(data=parkland_grouped |> mutate(park_label = "Parks"),
...
```{r}
access_WALK_i_neighbourhoods_df <- access_WALK_i_neighbourhoods %>% st_drop_geometry() |>
  mutate(CLASSIF9_fact = factor(CLASSIF9, 
                           levels = c("Emerging Neighbourhood","Neighbourhood Improvement Area","Not an NIA or Emerging Neighbourhood")))

df_long <- access_WALK_i_neighbourhoods_df %>%
  pivot_longer(cols = c(vv_tot_neigh, vv_opp_neigh),
               names_to = "type",
               values_to = "value") %>%
  mutate(
    type = recode(type,
                  vv_tot_neigh = "Total Constrained",
                  vv_opp_neigh = "Singly Constrained")
  )
```

```{r, eval=FALSE}
ggplot(df_long, aes(x = CLASSIF9_fact, y = value, fill = type)) +
  geom_boxplot(position = position_dodge(width = 0.75), alpha = 0.8, outlier.alpha = 0.3) +
  geom_hline(yintercept = 12, color = "red", linetype = "dashed", linewidth = 0.8) +
  scale_fill_brewer(palette = "Set2", name = "") +
    scale_y_log10(breaks = c(1, 2, 5, 10, 12, 20, 50, 100, 200), 
                  minor_breaks = NULL,
                  labels = scales::comma_format()) +
  labs(x = " ", y = "Accessible Parkland Area (sqm per capita) - log scale") +
  theme_minimal() 

ggsave("./figures/chp4-parkland_access_neighbourhood_percapita_WALK_boxplots.png", width = 10, height = 4, dpi = 300)
```
...
Finding diff in neighbourhood vv and V values:
```{r}
...
Adding a cat variable that states if below a threshold and if a NIA or EN.
```{r}
access_WALK_i_neighbourhoods <- access_WALK_i_neighbourhoods |> 
  mutate(cat_col = case_when(
#    CLASSIF9 != "Not an NIA or Emerging Neighbourhood" &
      (vv_opp_neigh < 12 & vv_tot_neigh < 12) ~ "Both",
#    CLASSIF9 != "Not an NIA or Emerging Neighbourhood" & 
      (vv_tot_neigh < 12) ~ "Only",
    TRUE ~ "Neither"))
```

Adding how many categories did the value change by? 
```{r}
access_WALK_i_neighbourhoods <- access_WALK_i_neighbourhoods |>
  mutate(cat_diff = as.numeric(vv_opp_neigh_cat) - as.numeric(vv_tot_neigh_cat),
         cat_diff = factor(cat_diff, levels = c(-2, -1, 0, 1, 2)))
```

```{r, eval=FALSE}
redpurple <-  c("#67001F", "#D6604D", "#DADAEB", "#807DBA", "#54278F") #brewer.pal(11, "RdBu")[c(1, 3)], brewer.pal(9, "Purples")[c(3,6,8)]

ggplot() +
  geom_sf(data = lake_ontario, fill="lightblue", col=NA, size = 0.3) +
      geom_sf(data = access_WALK_i_neighbourhoods,
          aes(fill = cat_diff), col = NA) +
  geom_sf_pattern(data = neighbourhoods, aes(pattern = factor(CLASSIF9)), fill= NA,
                  pattern_density = 0.15, pattern_spacing = 0.03, color = "grey30", size = 0.1, pattern_alpha = 0.4) +
  coord_sf(xlim = c(toronto_bbox["xmin"], toronto_bbox["xmax"]), 
           ylim = c(toronto_bbox["ymin"], toronto_bbox["ymax"]), 
  ...
  coord_sf(xlim = c(toronto_bbox["xmin"], toronto_bbox["xmax"]), 
           ...
           expand = FALSE) +
  scale_fill_manual(
    name = "Diff. in accessible per capita\nparkland area category\ndue to population competition:",
    values = redpurple,
    na.value = "white") +
  theme_void()+
  theme(plot.background = element_rect(fill = "white", color = NA),  
...
ggplot() +
  ...
  scale_fill_gradient2(
    name = "Diff. in per capita accessibility\ndue to population competition\n(sqm per capita):",
    midpoint=0,
    mid = "grey90",
    na.value = "white") +
  theme_void()+
...
parks_neigh_split <- parks_neigh_split |>
  (\(x) mutate(x, area_ha = as.numeric(st_area(x)) / 10000))() #the area of the portion of park in each neighbourhood
...
sum(park_area_summary$total_park_area_ha)  # total area counted across neighbourhoods... #should equal 7632.922 ha but w.e... it overshoots due to the splitting.
```
...
access_WALK_j <- access_WALK |> 
  group_by(P_ID)|>
  summarise(M_j_0 = sum(M_ji_0, na.rm=T),
            M_tot_j = sum(M_tot_ji, na.rm=T),
            kappahat_tot_j = sum(kappahat_tot_ji, na.rm=T),
            M_pop_j = sum(M_pop_ji, na.rm=T))
```
...
#now finding the capped 0,0.25, 0.5, and 0.75 and 1 percentiles for the caped distribution
breaks_M_j_0 <- quantile(access_WALK_j$M_j_0, probs = seq(0, 1, 0.25), na.rm = TRUE) 
breaks_M_tot_j <- quantile(access_WALK_j$M_tot_j, probs = seq(0, 1, 0.25), na.rm = TRUE) 
breaks_M_pop_j <- quantile(access_WALK_j$M_pop_j, probs = seq(0, 1, 0.25), na.rm = TRUE) 

#creating factor quartiles for the bins
access_WALK_j$M_j_0_quart <- cut(access_WALK_j$M_j_0, 
                                 breaks = breaks_M_j_0, 
                                 include.lowest = TRUE, 
                                 labels = c("Q1", "Q2", "Q3", "Q4"))

access_WALK_j$M_tot_j_quart <- cut(access_WALK_j$M_tot_j,
                                   breaks = breaks_M_tot_j, 
                                   include.lowest = TRUE, 
                                   labels = c("Q1", "Q2", "Q3", "Q4"))

access_WALK_j$M_pop_j_quart <- cut(access_WALK_j$M_pop_j,
                                   breaks = breaks_M_pop_j, 
                                   include.lowest = TRUE, 
                                   labels = c("Q1", "Q2", "Q3", "Q4"))
...
M_0_j_access_WALK_plot <- ggplot() +
  ...
  geom_sf(data = access_WALK_j,
          aes(fill = M_j_0_quart), col = NA) +
  coord_sf(xlim = c(toronto_bbox["xmin"], toronto_bbox["xmax"]), 
           ylim = c(toronto_bbox["ymin"], toronto_bbox["ymax"]), 
           expand = FALSE) +
  scale_fill_manual(values = setNames(reds, quartile_levels),
...
M_T_j_access_WALK_plot <- ggplot() +
  ...
  geom_sf(data = TO_boundary, col ="black", fill = NA, size=0.1) +
  geom_sf(data = access_WALK_j,
          aes(fill = M_tot_j_quart), col = NA) +
  coord_sf(xlim = c(toronto_bbox["xmin"], toronto_bbox["xmax"]), 
           ylim = c(toronto_bbox["ymin"], toronto_bbox["ymax"]), 
           expand = FALSE) +
    scale_fill_manual(values = setNames(greens, quartile_levels),
...
summary_long <- summary_by_size |>
  ...
  mutate(Measure = recode(Measure,
                     "mm_tot" = "Total Constrained",
                     "mm_pop" = "Singly Constrained"))
ggplot(summary_long, aes(x = reorder(park_size, -Value), y = Value, fill = Measure)) +
  geom_col(position = "dodge") +
  labs(y = "Accessible Population per Sqm of Parkland", x = "", fill = "") +
  theme_minimal()  +
...
rank_changes_parks <- test4 |>
  mutate(rank_tot = rank(-mm_tot_j, ties.method = "min"),
...
#creating factor quartiles for the bins
access_WALK_j_neighbourhoods$M_neigh_0_quart <- cut(access_WALK_j_neighbourhoods$M_neigh_0, 
                                 breaks = breaks_M_neigh_0, 
                                 include.lowest = TRUE, 
                                 labels = c("Q1", "Q2", "Q3", "Q4"))

access_WALK_j_neighbourhoods$M_tot_neigh_quart <- cut(access_WALK_j_neighbourhoods$M_tot_neigh,
                                   breaks = breaks_M_tot_neigh, 
                                   include.lowest = TRUE, 
                                   labels = c("Q1", "Q2", "Q3", "Q4"))

access_WALK_j_neighbourhoods$M_pop_neigh_quart <- cut(access_WALK_j_neighbourhoods$M_pop_neigh,
                                   breaks = breaks_M_pop_neigh, 
                                   include.lowest = TRUE, 
                                   labels = c("Q1", "Q2", "Q3", "Q4"))
...
M_0_neigh_access_WALK_plot <- ggplot() +
  ...
  geom_sf(data = access_WALK_j_neighbourhoods,
          aes(fill = M_neigh_0_quart), col = NA) +
  geom_sf_pattern(data = neighbourhoods, aes(pattern = factor(CLASSIF9)), fill= NA,
                  pattern_density = 0.15, pattern_spacing = 0.03, color = "grey30", size = 0.1, pattern_alpha = 0.4) +
  geom_sf(data = TO_boundary, col ="grey30", size = 6, fill = NA) +
  coord_sf(xlim = c(toronto_bbox["xmin"], toronto_bbox["xmax"]), 
...
M_T_neigh_access_WALK_plot <- ggplot() +
  geom_sf(data = lake_ontario, fill="lightblue", col=NA, size = 0.3) +
  geom_sf(data = access_WALK_j_neighbourhoods,
          aes(fill = M_tot_neigh_quart), col = NA) +
  geom_sf(data = TO_boundary, col ="grey30", size = 6, fill = NA) +
  geom_sf_pattern(data = neighbourhoods, aes(pattern = factor(CLASSIF9)), fill= NA,
                  pattern_density = 0.15, pattern_spacing = 0.03, color = "grey30", size = 0.1, pattern_alpha = 0.4) +
  coord_sf(xlim = c(toronto_bbox["xmin"], toronto_bbox["xmax"]), 
...
M_S_neigh_access_WALK_plot <- ggplot() +
  geom_sf(data = lake_ontario, fill="lightblue", col=NA, size = 0.3) +
  geom_sf(data = access_WALK_j_neighbourhoods,
          aes(fill = M_pop_neigh_quart), col = NA) +
  geom_sf(data = TO_boundary, col ="grey30", size = 6, fill = NA) +
  geom_sf_pattern(data = neighbourhoods, aes(pattern = factor(CLASSIF9)), fill= NA,
                  pattern_density = 0.15, pattern_spacing = 0.03, color = "grey30", size = 0.1, pattern_alpha = 0.4) +
  coord_sf(xlim = c(toronto_bbox["xmin"], toronto_bbox["xmax"]), 
  ...
  scale_fill_manual(values = setNames(greens, quartile_levels),
                    ...
                    na.value = "white",
                    guide = guide_legend(order = 1)) +  theme_void()+
  theme(plot.background = element_rect(fill = "white", color = NA),  
        panel.background = element_rect(fill = "white", color = NA),
        panel.grid = element_blank(), 
    legend.position = "right") +
...
```
save for discussion in chapter:
```{r,eval=FALSE}
save(access_WALK_j_neighbourhoods, file="./intermediate/access_WALK_j_neighbourhoods.rda")
save(access_WALK_j, file="./intermediate/access_WALK_j.rda")
```
...
access_WALK_j_neighbourhoods$mm_neigh_0_quart <- cut(access_WALK_j_neighbourhoods$mm_neigh_0, 
                                 breaks = breaks_mm_neigh_0, 
                                 include.lowest = TRUE, 
                                 labels = c("Q1", "Q2", "Q3", "Q4"))

access_WALK_j_neighbourhoods$mm_tot_neigh_quart <- cut(access_WALK_j_neighbourhoods$mm_tot_neigh,
                                   breaks = breaks_mm_tot_neigh, 
                                   include.lowest = TRUE, 
                                   labels = c("Q1", "Q2", "Q3", "Q4"))

access_WALK_j_neighbourhoods$mm_pop_neigh_quart <- cut(access_WALK_j_neighbourhoods$mm_pop_neigh,
                                   breaks = breaks_mm_pop_neigh, 
                                   include.lowest = TRUE, 
                                   labels = c("Q1", "Q2", "Q3", "Q4"))
...
mm_0_neigh_access_WALK_plot <- ggplot() +
  geom_sf(data = lake_ontario, fill="lightblue", col=NA, size = 0.3) +
  geom_sf(data = access_WALK_j_neighbourhoods,
          aes(fill = mm_neigh_0_quart), col = NA) +
  geom_sf_pattern(data = neighbourhoods, aes(pattern = factor(CLASSIF9)), fill= NA,
                  pattern_density = 0.15, pattern_spacing = 0.03, color = "grey30", size = 0.1, pattern_alpha = 0.4) +
  geom_sf(data = TO_boundary, col ="grey30", size = 6, fill = NA) +
  coord_sf(xlim = c(toronto_bbox["xmin"], toronto_bbox["xmax"]), 
           ylim = c(toronto_bbox["ymin"], toronto_bbox["ymax"]), 
           expand = FALSE) +
  scale_fill_manual(values = setNames(yellowred, quartile_levels),
                    name = "Population-≤15-min\nper Parkland Sqm",
...
mm_T_neigh_access_WALK_plot <- ggplot() +
  geom_sf(data = lake_ontario, fill="lightblue", col=NA, size = 0.3) +
  geom_sf(data = access_WALK_j_neighbourhoods,
          aes(fill = mm_tot_neigh_quart), col = NA) +
  geom_sf(data = TO_boundary, col ="grey30", size = 6, fill = NA) +
  geom_sf_pattern(data = neighbourhoods, aes(pattern = factor(CLASSIF9)), fill= NA,
                  pattern_density = 0.15, pattern_spacing = 0.03, color = "grey30", size = 0.1, pattern_alpha = 0.4) +
  coord_sf(xlim = c(toronto_bbox["xmin"], toronto_bbox["xmax"]), 
           ylim = c(toronto_bbox["ymin"], toronto_bbox["ymax"]), 
           expand = FALSE) +
  scale_fill_manual(values = setNames(yellowgreen, quartile_levels),
                    name = "Accessible Population\nper Parkland Sqm",
...
mm_S_neigh_access_WALK_plot <- ggplot() +
  geom_sf(data = lake_ontario, fill="lightblue", col=NA, size = 0.3) +
  geom_sf(data = access_WALK_j_neighbourhoods,
          aes(fill = mm_pop_neigh_quart), col = NA) +
  geom_sf(data = TO_boundary, col ="grey30", size = 6, fill = NA) +
  geom_sf_pattern(data = neighbourhoods, aes(pattern = factor(CLASSIF9)), fill= NA,
                  pattern_density = 0.15, pattern_spacing = 0.03, color = "grey30", size = 0.1, pattern_alpha = 0.4) +
  coord_sf(xlim = c(toronto_bbox["xmin"], toronto_bbox["xmax"]), 
