...
```{r}
load(file="./parkland_edge_ent_only_with_centroid_points.rda") #this is the dest file that was used to route the original ttms. 
...
parkland_edge_ent_only_with_centroid_points |> st_drop_geometry() |> filter(park_name == "NEWTONBROOK PARK")
...
parkland_edge_ent_only_with_centroid_points |> st_drop_geometry() |> filter(entrance_type == "Edge intersection") |> pull(P_piece_ID) |> unique() |> length()
parkland_edge_ent_only_with_centroid_points |> st_drop_geometry() |> filter(entrance_type == "Geometric centroid") |> pull(P_piece_ID) |> unique() |> length() #1408 is also the number of park pieces that have entrances -- it is missing the 550 pieces without. 
```
SO! Let's filter out these P_piece_ent_IDs that are equal to or above 5725
```{r}
wrong_centroid_entrance_points_TOREMOVE <- parkland_edge_ent_only_with_centroid_points |> filter(P_piece_ent_ID >= 5725)
...
```{r}
load("./ttm/ttm_BICYCLE_onlyENTwtestCENTtimes.rda")
...
comparison <- parkland_edge_ent_only_with_centroid_points |> st_drop_geometry() |> full_join(parkland_grouped |> select(c("P_ID","area_ha","park_size")), by = "P_ID")
...
ggplot(comparison |> na.omit(), aes(x = tt_CENT, y = tt_ENT, color = park_size)) +
  ...
  annotate("text",
    x = -Inf, y = Inf,
    label = paste("n =", nrow(comparison |> na.omit())),
    hjust = -0.3, vjust = 1.1, size = 4) +
  theme(legend.position = c(0.85, 0.2)) + 
  coord_equal()

ggsave("./figures/intermediate/chp2-scatter_tt_cent_vs_ent_BICYCLE_plot.png", width = 5, height = 5, dpi = 300)
```
...
```{r}
load("./ttm/ttm_WALK_onlyENTwtestCENTtimes.rda")
...
ggsave("./figures/intermediate/chp2-scatter_tt_cent_vs_ent_WALK_plot.png", width = 5, height = 5, dpi = 300)
```
...
```{r}
load("./ttm/ttm_TRANSIT_onlyENTwtestCENTtimes.rda")
...
ggsave("./figures/intermediate/chp2-scatter_tt_cent_vs_ent_TRANSIT_plot.png", width = 5, height = 5, dpi = 300)
```
...
ggsave("./figures/chp2-distribution_tt_cent_vs_ent_TRANSIT_plot.png", width = 7, height = 6, dpi = 300)
```
...
```{r}
load("./ttm/ttm_CAR_onlyENTwtestCENTtimes.rda")
...
ggsave("./figures/intermediate/chp2-scatter_tt_cent_vs_ent_CAR_plot.png", width = 5, height = 5, dpi = 300)
```
...
parkland_ONLY_centroid_entrance_points$P_piece_ent_ID |> unique() |> length() #should be 550 individual points (centroids per piece)
parkland_ONLY_centroid_entrance_points$P_piece_ID |> unique() |> length() #550 park pieces don't have entrances
parkland_ONLY_centroid_entrance_points$P_ID |> unique() |> length() #465 parks have a 'centroid' entrances, but some of these parks have pieces with entrances and at least 1 centroid (i.e., 65 parks).
(parkland_ONLY_centroid_entrance_points$P_ID |> unique() |> length()) - (list_P_IDs_with_pieces_both_Cent_Ent |> length()) #so, the total number of parks WITHOUT any edge entrance at all is 403. 

dest$id |> unique() |> length() #checks, should be 550 unique destinations (centroid entrances to park pieces)
```
...
ttm_WALK_onlyCENT$to_id |> unique() |> length() #X out of 550 entrances can be reached
...
ttm_BICYCLE_onlyCENT$to_id |> unique() |> length() #X out of 550 entrances can be reached
...
ttm_TRANSIT_onlyCENT$to_id |> unique() |> length() #X out of 550 entrances can be reached
...
ttm_CAR_onlyCENT$to_id |> unique() |> length() #X out of 550 entrances can be reached
