...
## 1. Unconstrained -- per DB by mode
```{r}
...
```{r}
access_allmodes_i_walk <- TO_census_21_DB |>
  left_join(access_allmodes_i_m |> filter(mode == "Walk"), by=c("DB_UID" = "from_id")) |>
...
access_allmodes_i_transit <- TO_census_21_DB |>
  left_join(access_allmodes_i_m |> filter(mode == "Transit"), by=c("DB_UID" = "from_id")) |>
   mutate( V_i_0_m= replace_na(V_i_0_m, 0),
           V_tot_i_m= replace_na(V_tot_i_m, 0),
           kappa_tot_i_m = replace_na(kappa_tot_i_m, 0),
           V_opp_i_m= replace_na(V_opp_i_m, 0),
...
access_allmodes_i_car <- TO_census_21_DB |>
  left_join(access_allmodes_i_m |> filter(mode == "Car"), by=c("DB_UID" = "from_id")) |>
   mutate( V_i_0_m= replace_na(V_i_0_m, 0),
           V_tot_i_m= replace_na(V_tot_i_m, 0),
           kappa_tot_i_m = replace_na(kappa_tot_i_m, 0),
           V_opp_i_m= replace_na(V_opp_i_m, 0),
...
access_allmodes_i_bicycle <- TO_census_21_DB |>
  left_join(access_allmodes_i_m |> filter(mode == "Bicycle"), by=c("DB_UID" = "from_id")) |>
   mutate( V_i_0_m= replace_na(V_i_0_m, 0),
           V_tot_i_m= replace_na(V_tot_i_m, 0),
           kappa_tot_i_m = replace_na(kappa_tot_i_m, 0),
           V_opp_i_m= replace_na(V_opp_i_m, 0),
...
```{r}
breaks_V_i_0 <- quantile(access_allmodes_i_m|> filter(V_i_0_m > 0) |> pull(V_i_0_m), probs = seq(0, 1, 0.1), na.rm = TRUE)
breaks_V_tot_i <- quantile(access_allmodes_i_m |> filter(V_tot_i_m > 0) |> pull(V_tot_i_m), probs = seq(0, 1, 0.1), na.rm = TRUE) 
breaks_V_opp_i <- quantile(access_allmodes_i_m |> filter(V_opp_i_m > 0) |> pull(V_opp_i_m), probs = seq(0, 1, 0.1), na.rm = TRUE) 

access_allmodes_i_m$V_i_0_m_quart <- cut(access_allmodes_i_m$V_i_0_m, 
                                 breaks = breaks_V_i_0, 
                                 include.lowest = TRUE, 
                                 labels = c("Q1", "Q2", "Q3", "Q4", "Q5","Q6", "Q7", "Q8", "Q9", "Q10"))

access_allmodes_i_m$V_tot_i_m_quart <- cut(access_allmodes_i_m$V_tot_i_m,
                                   breaks = breaks_V_tot_i, 
                                   include.lowest = TRUE, 
                                   labels = c("Q1", "Q2", "Q3", "Q4", "Q5","Q6", "Q7", "Q8", "Q9", "Q10"))

access_allmodes_i_m$V_opp_i_m_quart <- cut(access_allmodes_i_m$V_opp_i_m,
                                   breaks = breaks_V_opp_i, 
                                   include.lowest = TRUE, 
                                   labels = c("Q1", "Q2", "Q3", "Q4", "Q5","Q6", "Q7", "Q8", "Q9", "Q10"))

#labels
legend_labels_V_i_0 <- paste0(round(breaks_V_i_0[-length(breaks_V_i_0)], 4), "–",
  round(breaks_V_i_0[-1], 4))
legend_labels_V_tot_i <- paste0(round(breaks_V_tot_i[-length(breaks_V_tot_i)], 6), "–",
  round(breaks_V_tot_i[-1], 6))
legend_labels_V_opp_i<- paste0(round(breaks_V_opp_i[-length(breaks_V_opp_i)], 6), "–",
  round(breaks_V_opp_i[-1], 6))

quartile_levels_10 <- c("Q1", "Q2", "Q3", "Q4", "Q5","Q6", "Q7", "Q8", "Q9", "Q10")
```

```{r,eval=FALSE}
oranges10 <- brewer.pal(9, "Oranges")[c(3:9)]
oranges10 <- colorRampPalette(oranges10)(length(quartile_levels_10)) 
...
V_0_i_access_m_plot <- ggplot() +
  ...
  geom_sf(data = TO_census_21_DB |> filter(Population==0) |> 
            mutate(pop_label = "No population"),
           aes(fill = pop_label), col= NA) + #there is NO population
  geom_sf(data = TO_census_21_DB |> filter(Population!=0) |>
            mutate(pop_label = "No accessibility"), 
          aes(fill = pop_label), col= NA) + #there IS population
scale_fill_manual(values = c("No accessibility" = "grey70",
                               "No population" = "grey95"),
                    name = "",
                    guide = guide_legend(order = 2)) +
  new_scale_fill()+
  geom_sf(data = access_allmodes_i_m |> filter(V_i_0_m > 0), aes(fill = V_i_0_m_quart), col = NA) +
  geom_sf(data = TO_boundary, col ="grey30", size = 6, fill = NA) +
  scale_fill_manual(values = setNames(oranges10, quartile_levels_10),
                    name = "Parkland-ha-by-\nmodal-impedance",
  ...
  scale_color_manual(values = c(" " = "black"), name = "Expressway", guide = guide_legend(order = 5)) + 
  geom_sf(data = ttc_subway, aes(linetype = ROUTE_NAME), size = 0.8) +
  scale_linetype_manual(name = "TTC Subway Route", values = linetypes, guide = guide_legend(order = 4)) +
  geom_sf(data=parkland_grouped |> mutate(park_label = "Parks"),
          aes(fill=park_label), col=NA, alpha=0.7)+
  scale_fill_manual(values = c("Parks" = "forestgreen"),
                    name = "",
...
```
## 2. Constrained -- per DB -- modes summed to 1 DB
```{r}
access_allmodes_i <- TO_census_21_DB |> 
  left_join(access_allmodes_i_m |> st_drop_geometry() |> group_by(DB_UID) |>
              summarise(V_tot_i = sum(V_tot_i_m, na.rm=T),
                        V_opp_i = sum(V_opp_i_m, na.rm=T)),
            by=c("DB_UID")) |>
   mutate( V_tot_i= replace_na(V_tot_i, 0),
           V_opp_i= replace_na(V_opp_i, 0))
```

```{r}
breaks_V_tot_i_full <- quantile(access_allmodes_i |> filter(V_tot_i > 0) |> pull(V_tot_i), probs = seq(0, 1, 0.1), na.rm = TRUE) 
breaks_V_opp_i_full <- quantile(access_allmodes_i |> filter(V_opp_i > 0) |> pull(V_opp_i), probs = seq(0, 1, 0.1), na.rm = TRUE) 

access_allmodes_i$V_tot_i_quart <- cut(access_allmodes_i$V_tot_i,
                                   breaks = breaks_V_tot_i_full, 
                                   include.lowest = TRUE, 
                                   labels = c("Q1", "Q2", "Q3", "Q4", "Q5","Q6", "Q7", "Q8", "Q9", "Q10"))

access_allmodes_i$V_opp_i_quart <- cut(access_allmodes_i$V_opp_i,
                                   breaks = breaks_V_opp_i_full, 
                                   include.lowest = TRUE, 
                                   labels = c("Q1", "Q2", "Q3", "Q4", "Q5","Q6", "Q7", "Q8", "Q9", "Q10"))

#labels
legend_labels_V_tot_i_full <- paste0(round(breaks_V_tot_i_full[-length(breaks_V_tot_i_full)], 4), "–",
  round(breaks_V_tot_i_full[-1], 4))

legend_labels_V_opp_i_full<- paste0(round(breaks_V_opp_i_full[-length(breaks_V_opp_i_full)], 4), "–",
  round(breaks_V_opp_i_full[-1], 4))

quartile_levels_10 <- c("Q1", "Q2", "Q3", "Q4", "Q5","Q6", "Q7", "Q8", "Q9", "Q10")
```

```{r,eval=FALSE}
purples10 <- brewer.pal(9, "Purples")[c(3:9)]
purples10 <- colorRampPalette(purples10)(length(quartile_levels_10)) 

V_T_i_access_plot <- ggplot() +
  geom_sf(data = lake_ontario, fill="lightblue", col=NA, size = 0.3) +
  ...
  geom_sf(data = TO_census_21_DB |> filter(Population!=0) |>
            mutate(pop_label = "No accessibility"), 
          aes(fill = pop_label), col= NA) + #there IS population
scale_fill_manual(values = c("No accessibility" = "grey80",
                               "No population" = "grey95"),
                    name = "",
                    guide = guide_legend(order = 2)) +
  new_scale_fill() +
  geom_sf(data = access_allmodes_i |> filter(V_tot_i > 0), aes(fill = V_tot_i_quart), col = NA) +
  geom_sf(data = TO_boundary, col ="grey30", size = 6, fill = NA) +
  scale_fill_manual(values = setNames(purples10, quartile_levels_10),
                    name = "Multimodal Accessible Parkland (ha)",
                    labels = legend_labels_V_tot_i_full,
                    na.value = NA,
                    guide = guide_legend(order = 1)) +
  new_scale_fill() +
  geom_sf(data = TO_highspeed_roads, aes(color = " "), size=0.4)+
...
scale_fill_manual(values = c("No accessibility" = "grey80",
                    ...
                    guide = guide_legend(order = 2)) +
  new_scale_fill() +
  geom_sf(data = access_allmodes_i |> filter(V_opp_i > 0),
          aes(fill = V_opp_i_quart), col = NA) +
  geom_sf(data = TO_boundary, col ="grey30", size = 6, fill = NA) +
  scale_fill_manual(values = setNames(purples10, quartile_levels_10),
                    name = "Multimodal Accessible Parkland (ha)",
                    labels = legend_labels_V_opp_i_full,
                    na.value = NA,
                    guide = guide_legend(order = 1)) +
  new_scale_fill() +
  geom_sf(data = TO_highspeed_roads, aes(color = " "), size=0.4)+
...
```{r,eval=FALSE}
V_T_S_i_access_plot <- V_T_i_access_plot / V_S_i_access_plot
V_T_S_i_access_plot
ggsave("./figures/chp5-mm_parkland_conc_access_DB_plots_summedmodes.png", width = 10, height = 10, dpi = 300)
```
## 3. Constrained -- per DB and per mode
...
scale_fill_manual(values = c("No accessibility" = "grey80",
                    ...
                    guide = guide_legend(order = 2)) +
  new_scale_fill() +
  geom_sf(data = access_allmodes_i_m |> filter(V_tot_i_m > 0), aes(fill = V_tot_i_m_quart), col = NA) +
  geom_sf(data = TO_boundary, col ="grey30", size = 6, fill = NA) +
  scale_fill_manual(values = setNames(purples10, quartile_levels_10),
...
scale_fill_manual(values = c("No accessibility" = "grey80",
                    ...
                    guide = guide_legend(order = 2)) +
  new_scale_fill() +
  geom_sf(data = access_allmodes_i_m |> filter(V_opp_i_m > 0), aes(fill = V_opp_i_m_quart), col = NA) +
  geom_sf(data = TO_boundary, col ="grey30", size = 6, fill = NA) +
  scale_fill_manual(values = setNames(purples10, quartile_levels_10),
                    name = "Accessible Parkland (ha)",
...
now, let's make a different plot for each mode -- comparing V_opp - V_tot per mode.
```{r}
access_allmodes_i_m_no0 <- access_allmodes_i_m |> filter(V_opp_i_m != 0 & V_tot_i_m != 0) |>
  mutate(diff = V_opp_i_m - V_tot_i_m) |> filter(diff != 0)

quantile_breaks <- quantile(access_allmodes_i_m_no0$diff, probs = seq(0, 1, by = 0.1), na.rm = TRUE)

access_allmodes_i_m_no0 <- access_allmodes_i_m_no0 |>
  mutate(diff_q = cut(diff, breaks = quantile_breaks, include.lowest = TRUE, dig.lab = 5,
                      labels = scales::label_number(accuracy = 0.001)(head(quantile_breaks, -1)) |>
                        paste("to", scales::label_number(accuracy = 0.001)(tail(quantile_breaks, -1)))))
...
```{r,eval=FALSE}
purpleorange10 <- brewer.pal(10, "Spectral") 
#purpleorange10 <- colorRampPalette(purpleorange10)(30) 
purpleorange10 <- c("#9E0142","#D1394E","#EF6644","#F57848", "#FBA05A", "#FDC272", "#FDD27F", "#9F96C3", "#6D539C", "#41146B")


linetypes <- c("dashed", "dotted", "dotdash", "longdash")


diff_V_S_to_T_i_access_m_plot <- ggplot() +
  geom_sf(data = lake_ontario, fill="lightblue", col=NA, size = 0.3) +
...
scale_fill_manual(values = c("No accessibility" = "grey80",
                    ...
                    guide = guide_legend(order = 2)) +
  new_scale_fill() +
  geom_sf(data = access_allmodes_i_m_no0, aes(fill = diff_q), col = NA) +
  geom_sf(data = TO_boundary, col ="grey30", size = 6, fill = NA) +
  scale_fill_manual(values  = purpleorange10, name = "Diff. in multimodal\naccessible parkland\n area (ha) (Singly - Total)",
                    na.value = NA,
                    guide = guide_legend(order = 1)) +
  new_scale_fill() +
  geom_sf(data = TO_highspeed_roads, aes(color = " "), size=0.4)+
...
diff_V_S_to_T_i_access_m_plot
ggsave("./figures/chp5-mm_parkland_diff_conc_access_DB_plots.png", width = 10, height = 7, dpi = 300)
```

## 4. Motorized - constrained per capita by mode

Calculating quartile (for mapping):
```{r}
access_allmodes_i_m_motorized <- access_allmodes_i_m |>
  filter(mode %in% c("Car", "Transit"))

custom_breaks <- c(0,1, 2, 4, 12, 28, 50, Inf)
custom_labels <- c("0–1", "1-2","2–4 (Hula Hoop)", "4–12 (Patio Umbrella)", "12–28 (Bus Shelter)", "28-50 (Mid-size Tree Canopy)", "50+ (Large Tree Canopy)")

access_allmodes_i_m_motorized_no0 <- access_allmodes_i_m_motorized |> filter(vv_tot_i_m !=0 | vv_opp_i_m !=0) |>
  mutate(vv_tot_i_m_quart = cut(vv_tot_i_m, breaks = custom_breaks,
                                labels = custom_labels, include.lowest = TRUE),
    vv_opp_i_m_quart = cut(vv_opp_i_m, breaks = custom_breaks, 
                           labels = custom_labels, include.lowest = TRUE))

quartile_levels_custom <- custom_labels
```

```{r,eval=FALSE}
plasma5 <- viridisLite::plasma(7, direction = -1) 
linetypes <- c("dashed", "dotted", "dotdash", "longdash")

vv_T_i_access_m_motor_plot <- ggplot() +
  geom_sf(data = lake_ontario, fill="lightblue", col=NA, size = 0.3) +
  geom_sf(data = TO_census_21_DB |> filter(Population==0) |> 
            mutate(pop_label = "No population"),
           aes(fill = pop_label), col= NA) + #there is NO population
...
scale_fill_manual(values = c("No accessibility" = "grey80",
                    ...
                    guide = guide_legend(order = 2)) +
  new_scale_fill() +
  geom_sf(data = access_allmodes_i_m_motorized |> filter(vv_tot_i_m > 0), aes(fill = vv_tot_i_m_quart), col = NA, alpha=0.8) +
  scale_fill_manual(values = setNames(plasma5, quartile_levels_custom),
                    name = "Accessible Parkland (ha)\nPer Mode-Using Capita",
                    labels = quartile_levels_custom,
                    na.value = NA,
                    guide = guide_legend(order = 1)) +
  new_scale_fill() +
    geom_sf(data = TO_boundary, col ="grey30", size = 6, fill = NA) +
...
vv_S_i_access_m_motor_plot <- ggplot() +
  geom_sf(data = lake_ontario, fill="lightblue", col=NA, size = 0.3) +
  geom_sf(data = TO_census_21_DB |> filter(Population==0) |> 
            mutate(pop_label = "No population"),
           aes(fill = pop_label), col= NA) + #there is NO population
...
scale_fill_manual(values = c("No accessibility" = "grey80",
                    ...
                    guide = guide_legend(order = 2)) +
  new_scale_fill() +
  geom_sf(data = access_allmodes_i_m_motorized |> filter(vv_opp_i_m > 0), aes(fill = vv_opp_i_m_quart), col = NA, alpha=0.8) +
  geom_sf(data = TO_boundary, col ="grey30", size = 6, fill = NA) +
  scale_fill_manual(values = setNames(plasma5, quartile_levels_custom),
                    name = "Accessible Parkland (ha)\nPer Mode-Using Capita",
                    labels = quartile_levels_custom,
...
vv_T_S_i_access_m_motor_plot <- vv_T_i_access_m_motor_plot / vv_S_i_access_m_motor_plot

vv_T_S_i_access_m_motor_plot
ggsave("./figures/chp5-mm_parkland_motorised_per_capita_conc_access_DB_plots.png", width = 10, height = 7, dpi = 300)
```
### 4.1. their LISA
Let's plot their LISAs:
```{r, eval=FALSE}
access_allmodes_i_m_motorized_transit <- access_allmodes_i_m_motorized |>
  filter(mode == "Transit") |>
  mutate(vv_tot_i_m_capped = ifelse(vv_tot_i_m >= 28, 28, vv_tot_i_m),
         vv_opp_i_m_capped = ifelse(vv_opp_i_m >= 28, 28, vv_opp_i_m))

access_allmodes_i_m_motorized_transit_proj <- st_transform(access_allmodes_i_m_motorized_transit, 32617)
centroids <- st_centroid(access_allmodes_i_m_motorized_transit_proj)
coords <- st_coordinates(centroids)
nb <- dnearneigh(coords, d1 = 0, d2 = 2000)
lw <- nb2listw(nb, style = "W", zero.policy = TRUE)

lisa_tot <- localmoran(access_allmodes_i_m_motorized_transit$vv_tot_i_m_capped, lw, zero.policy = TRUE)
lisa_opp <- localmoran(access_allmodes_i_m_motorized_transit$vv_opp_i_m_capped, lw, zero.policy = TRUE)

lag_tot <- lag.listw(lw, access_allmodes_i_m_motorized_transit$vv_tot_i_m_capped)
lag_opp <- lag.listw(lw, access_allmodes_i_m_motorized_transit$vv_opp_i_m_capped)

access_allmodes_i_m_motorized_transit$cluster_tot <- NA
access_allmodes_i_m_motorized_transit$cluster_opp <- NA

access_allmodes_i_m_motorized_transit$cluster_tot <- case_when(
  access_allmodes_i_m_motorized_transit$vv_tot_i_m_capped > mean(access_allmodes_i_m_motorized_transit$vv_tot_i_m_capped,
                                                                     na.rm = TRUE) &
...
access_allmodes_i_m_motorized_transit$cluster_opp <- case_when(
  access_allmodes_i_m_motorized_transit$vv_opp_i_m_capped > mean(access_allmodes_i_m_motorized_transit$vv_opp_i_m_capped, na.rm = TRUE) &
...
transit_LISA_total_m_plot <- ggplot() +
  geom_sf(data = lake_ontario, fill="lightblue", col=NA) +
  geom_sf(data = TO_census_21_DB, color = NA, fill = "white") +
  geom_sf(data = access_allmodes_i_m_motorized_transit, aes(fill = cluster_tot), color = NA) +
...
transit_LISA_opp_m_plot <- ggplot() +
  geom_sf(data = lake_ontario, fill="lightblue", col=NA) +
  geom_sf(data = TO_census_21_DB, color = NA, fill = "white") +
  geom_sf(data = access_allmodes_i_m_motorized_transit, aes(fill = cluster_opp), color = NA) +
  scale_fill_manual(values = c(
    "High-High" = "red",
    "Low-Low" = "blue",
    "High-Low" = "orange",
...
```{r}
access_allmodes_i_m_motorized_car <- access_allmodes_i_m_motorized |>
  filter(mode == "Car") |>
  mutate(vv_tot_i_m_capped = ifelse(vv_tot_i_m >= 28, 28, vv_tot_i_m),
         vv_opp_i_m_capped = ifelse(vv_opp_i_m >= 28, 28, vv_opp_i_m))

access_allmodes_i_m_motorized_car_proj <- st_transform(access_allmodes_i_m_motorized_car, 32617)
centroids <- st_centroid(access_allmodes_i_m_motorized_car_proj)
coords <- st_coordinates(centroids)
nb <- dnearneigh(coords, d1 = 0, d2 = 500)
lw <- nb2listw(nb, style = "W", zero.policy = TRUE)

lisa_tot <- localmoran(access_allmodes_i_m_motorized_car$vv_tot_i_m_capped, lw, zero.policy = TRUE)
lisa_opp <- localmoran(access_allmodes_i_m_motorized_car$vv_opp_i_m_capped, lw, zero.policy = TRUE)

lag_tot <- lag.listw(lw, access_allmodes_i_m_motorized_car$vv_tot_i_m_capped)
lag_opp <- lag.listw(lw, access_allmodes_i_m_motorized_car$vv_opp_i_m_capped)

access_allmodes_i_m_motorized_car$cluster_tot <- NA
access_allmodes_i_m_motorized_car$cluster_opp <- NA

access_allmodes_i_m_motorized_car$cluster_tot <- case_when(
  access_allmodes_i_m_motorized_car$vv_tot_i_m_capped >
    mean(access_allmodes_i_m_motorized_car$vv_tot_i_m_capped,na.rm = TRUE) &
    lag_tot > mean(lag_tot, na.rm = TRUE) & lisa_tot[, 5] < 0.05 ~ "High-High",
...
# Repeating for singly constrained
access_allmodes_i_m_motorized_car$cluster_opp <- case_when(
  access_allmodes_i_m_motorized_car$vv_opp_i_m_capped > mean(access_allmodes_i_m_motorized_car$vv_opp_i_m_capped, na.rm = TRUE) &
    lag_opp > mean(lag_opp, na.rm = TRUE) & lisa_opp[, 5] < 0.05 ~ "High-High",
  access_allmodes_i_m_motorized_car$vv_opp_i_m_capped < mean(access_allmodes_i_m_motorized_car$vv_opp_i_m_capped, na.rm = TRUE) &
    lag_opp < mean(lag_opp, na.rm = TRUE) & lisa_opp[, 5] < 0.05 ~ "Low-Low",
  access_allmodes_i_m_motorized_car$vv_opp_i_m_capped > mean(access_allmodes_i_m_motorized_car$vv_opp_i_m_capped, na.rm = TRUE) &
...
car_LISA_total_m_plot <- ggplot() +
  geom_sf(data = lake_ontario, fill="lightblue", col=NA) +
  geom_sf(data = TO_census_21_DB, color = NA, fill = "white") +
  geom_sf(data = access_allmodes_i_m_motorized_car, aes(fill = cluster_tot), color = NA) +
  scale_fill_manual(values = c(
    "High-High" = "red",
    "Low-Low" = "blue",
    "High-Low" = "orange",
...
car_LISA_opp_m_plot <- ggplot() +
  geom_sf(data = lake_ontario, fill="lightblue", col=NA) +
  geom_sf(data = TO_census_21_DB, color = NA, fill = "white") +
  geom_sf(data = access_allmodes_i_m_motorized_car, aes(fill = cluster_opp), color = NA) +
  scale_fill_manual(values = c(
    "High-High" = "red",
    "Low-Low" = "blue",
    "High-Low" = "orange",
...
```{r}
car_LISA_total_m_plot
car_LISA_opp_m_plot
```

```{r, eval=FALSE}
LISA_allmotorized_m_plots <-
  (transit_LISA_total_m_plot + theme(legend.position = "none")) + 
  (car_LISA_total_m_plot + theme(legend.position = "right")) +
  (transit_LISA_opp_m_plot + theme(legend.position = "none"))+ 
  (car_LISA_opp_m_plot + theme(legend.position = "none")) +
  plot_layout(ncol = 2, nrow = 2)

LISA_allmotorized_m_plots
ggsave("./figures/chp5-LISA_allmotorized_m_plots.png", width = 10, height = 7, dpi = 300)
```

## 4. Non-motorized - constrained per capita by mode

Calculating quartile (for mapping):
```{r}
access_allmodes_i_m_nonmotorized <- access_allmodes_i_m |>
  filter(mode %in% c("Walk", "Bicycle"))

custom_breaks <- c(0,1, 2, 4, 12, 28, 50, Inf)
custom_labels <- c("0–1", "1-2","2–4 (Hula Hoop)", "4–12 (Patio Umbrella)", "12–28 (Bus Shelter)", "28-50 (Mid-size Tree Canopy)", "50+ (Large Tree Canopy)")

access_allmodes_i_m_nonmotorized <- access_allmodes_i_m_nonmotorized |>
  mutate(vv_tot_i_m_quart = cut(vv_tot_i_m, breaks = custom_breaks,
                                labels = custom_labels, include.lowest = TRUE),
    vv_opp_i_m_quart = cut(vv_opp_i_m, breaks = custom_breaks, 
                           labels = custom_labels, include.lowest = TRUE))
...
vv_T_i_access_m_nonmotor_plot <- ggplot() +
  geom_sf(data = lake_ontario, fill="lightblue", col=NA, size = 0.3) +
  geom_sf(data = TO_census_21_DB |> filter(Population==0) |> 
            mutate(pop_label = "No population"),
           aes(fill = pop_label), col= NA) + #there is NO population
...
scale_fill_manual(values = c("No accessibility" = "grey80",
                    ...
                    guide = guide_legend(order = 2)) +
  new_scale_fill() +
  geom_sf(data = access_allmodes_i_m_nonmotorized |> filter(vv_tot_i_m > 0), aes(fill = vv_tot_i_m_quart), col = NA, alpha=0.8) +
  geom_sf(data = TO_boundary, col ="grey30", size = 6, fill = NA) +
  scale_fill_manual(values = setNames(plasma5, quartile_levels_custom),
...
vv_S_i_access_m_nonmotor_plot <- ggplot() +
  geom_sf(data = lake_ontario, fill="lightblue", col=NA, size = 0.3) +
  geom_sf(data = TO_census_21_DB |> filter(Population==0) |> 
            mutate(pop_label = "No population"),
           aes(fill = pop_label), col= NA) + #there is NO population
...
scale_fill_manual(values = c("No accessibility" = "grey80",
                    ...
                    guide = guide_legend(order = 2)) +
  new_scale_fill() +
  geom_sf(data = access_allmodes_i_m_nonmotorized |> filter(vv_opp_i_m > 0), aes(fill = vv_opp_i_m_quart), col = NA, alpha=0.8) +
  geom_sf(data = TO_boundary, col ="grey30", size = 6, fill = NA) +
  scale_fill_manual(values = setNames(plasma5, quartile_levels_custom),
                    name = "Accessible Parkland (sqm)\nPer Mode-Using Capita",
                    labels = quartile_levels_custom,
...
vv_T_S_i_access_m_nonmotor_plot <- vv_T_i_access_m_nonmotor_plot / vv_S_i_access_m_nonmotor_plot
```

```{r,eval=FALSE}
vv_T_S_i_access_m_nonmotor_plot
ggsave("./figures/chp5-mm_parkland_nonmotorised_per_capita_conc_access_DB_plots.png", width = 10, height = 7, dpi = 300)
```
# Accessibility to population plots!

## 1. Unconstrained per park:
```{r}
...
access_allmodes_j_walk <- parkland_grouped |> left_join(access_allmodes_j_m |> filter(mode == "Walk") |> select(-c("area_ha")),
  ...
  mutate( M_j_0_m= replace_na(M_j_0_m, 0),
          ...
          park_m = replace_na(park_m, 0),
           mode= if_else(is.na(mode), "Walk", mode),
           mm_tot_i_m = ifelse(M_tot_j_m == 0 | park_m == 0, 0, M_tot_j_m/park_m*10000),
           mm_opp_i_m = ifelse(M_pop_j_m == 0 | park_m == 0, 0, M_pop_j_m/park_m*10000))
...
access_allmodes_j_car <- parkland_grouped |> left_join(access_allmodes_j_m |> filter(mode == "Car")|> select(-c("area_ha")),
   ...
   mutate( M_j_0_m= replace_na(M_j_0_m, 0),
          ...
          park_m = replace_na(park_m, 0),
           mode= if_else(is.na(mode), "Car", mode),
           mm_tot_j_m = ifelse(M_tot_j_m == 0 | park_m == 0, 0, M_tot_j_m/park_m*10000),
           mm_opp_j_m = ifelse(M_pop_j_m == 0 | park_m == 0, 0, M_pop_j_m/park_m*10000))
...
```{r}
breaks_M_j_0_m <- quantile(access_allmodes_j_m |> filter(M_j_0_m >0) |> pull(M_j_0_m), probs = seq(0, 1, 0.1), na.rm = TRUE) 
breaks_M_tot_j_m <- quantile(access_allmodes_j_m |> filter(M_tot_j_m>0) |> pull(M_tot_j_m) , probs = seq(0, 1, 0.1), na.rm = TRUE) 
breaks_M_pop_j_m <- quantile(access_allmodes_j_m |> filter(M_pop_j_m>0) |> pull(M_pop_j_m), probs = seq(0, 1, 0.1), na.rm = TRUE) 

access_allmodes_j_m$M_j_0_m_quart <- cut(access_allmodes_j_m$M_j_0_m, 
                                 breaks = breaks_M_j_0_m, 
                                 include.lowest = TRUE, 
                                 labels = c("Q1", "Q2", "Q3", "Q4", "Q5","Q6", "Q7", "Q8", "Q9", "Q10"))

access_allmodes_j_m$M_tot_j_m_quart <- cut(access_allmodes_j_m$M_tot_j_m,
                                   breaks = breaks_M_tot_j_m, 
                                   include.lowest = TRUE, 
                                   labels = c("Q1", "Q2", "Q3", "Q4", "Q5","Q6", "Q7", "Q8", "Q9", "Q10"))

access_allmodes_j_m$M_pop_j_m_quart <- cut(access_allmodes_j_m$M_pop_j_m,
                                   breaks = breaks_M_pop_j_m, 
                                   include.lowest = TRUE, 
                                   labels = c("Q1", "Q2", "Q3", "Q4", "Q5","Q6", "Q7", "Q8", "Q9", "Q10"))

#labels
legend_labels_M_j_0_m <- paste0(round(breaks_M_j_0_m[-length(breaks_M_j_0_m)], 1), "–",
  round(breaks_M_j_0_m[-1], 1))
legend_labels_M_tot_j_m <- paste0(round(breaks_M_tot_j_m[-length(breaks_M_tot_j_m)], 2), "–",
  round(breaks_M_tot_j_m[-1], 2))
legend_labels_M_pop_j_m <- paste0(round(breaks_M_pop_j_m[-length(breaks_M_pop_j_m)], 5), "–",
  round(breaks_M_pop_j_m[-1], 5))
...
Plot for accessibilities for unconstrained populations -- at DBs:
```{r, eval=FALSE}
oranges10 <- brewer.pal(9, "Oranges")[c(3:9)]
oranges10 <- colorRampPalette(oranges10)(length(quartile_levels_10)) 
linetypes <- c("dashed", "dotted", "dotdash", "longdash")

M_0_j_access_m_plot <- ggplot() +
  geom_sf(data = lake_ontario, fill="lightblue", col=NA, size = 0.3) +
  
  geom_sf(data = TO_census_21_DB, aes(fill = Population), col= NA) + 
  scale_fill_distiller(palette = "Greys", direction = 1, name = "Population") +
  new_scale_fill()+
  ...
  geom_sf(data = access_allmodes_j_m |> filter(M_j_0_m == 0),
          fill = "forestgreen", col = NA) +
  
  geom_sf(data = access_allmodes_j_m |> filter(M_j_0_m > 0),
          aes(fill = M_j_0_m_quart), col = NA) +
  scale_fill_manual(values = setNames(oranges10, quartile_levels_10),
...
M_0_j_access_m_plot
ggsave("./figures/chp5-pop_unconc_access_parks_allmodes_plots.png", width = 10, height = 7, dpi = 300)
```
...
M_T_j_access_allmodes_plot <- ggplot() +
  ...
  geom_sf(data = access_allmodes_j_m |> filter(M_tot_j_m == 0),
          fill = "forestgreen", col = NA) +
  
  geom_sf(data = access_allmodes_j_m |> filter(M_tot_j_m > 0),
          aes(fill = M_tot_j_m_quart), col = NA) +
  scale_fill_manual(values = setNames(purples10, quartile_levels_10),
...
M_S_j_access_allmodes_plot <- ggplot() +
  ...
  geom_sf(data = access_allmodes_j_m |> filter(M_pop_j_m == 0),
          fill = "forestgreen", col = NA) +
  
  geom_sf(data = access_allmodes_j_m |> filter(M_pop_j_m > 0),
          aes(fill = M_pop_j_m_quart), col = NA) +
  scale_fill_manual(values = setNames(purples10, quartile_levels_10),
                    name = "Accessible Population",
...
parkland_wboundaries <- parkland_grouped |> st_make_valid() |> 
  st_join(former_muni_boundaries,
          join = st_intersects,
          largest = TRUE) |> distinct(P_ID, .keep_all = TRUE)  #one row per park


TO_DBs_wboundaries <- TO_census_21_DB |> st_make_valid() |> 
  st_join(former_muni_boundaries,
          join = st_within,
          largest = TRUE) |> distinct(DB_UID, .keep_all = TRUE)  #one row per park
...
access_allmodes_muni_m <- access_allmodes_muni_m |>
  group_by(AREA_NA8, mode) |> summarise(M_pop = sum(M_pop_j_m),
                                        M_tot = sum(M_tot_j_m),
...
access_allmodes_muni_m <- access_allmodes_muni_m |>
  mutate(x_pos = as.numeric(factor(AREA_NA8, levels = neighbourhood_levels))) |>
  mutate(mm_opp = M_pop/prop_parkarea_m,
         mm_tot = M_tot/prop_parkarea_m)
access_allmodes_muni_m$mode <- factor(access_allmodes_muni_m$mode)

access_allmodes_muni_m <- access_allmodes_muni_m |> left_join(real_total_pop |> select(-c("x_pos")), by = "AREA_NA8")

access_allmodes_muni_m <- access_allmodes_muni_m |>
  mutate(real_pop_park_ratio = real_pop/real_parkarea)
```
...
```{r}
access_allmodes_muni_m$M_pop |> sum()
access_allmodes_muni_m$M_tot |> sum() #(diff due to  to )
...
Plots of amount of accessible population and actual  area per muni:
```{r, eval=FALSE}
Set1 <- brewer.pal(4, "Set1")
Set2 <- brewer.pal(4, "Set2")
...
```{r}
mm_T_muni_plot <- ggplot(access_allmodes_muni_m, aes(x = AREA_NA8, y = mm_tot, fill = mode)) +
  geom_bar(stat = "identity", position = position_dodge(width = 0.8)) +
  geom_point(
    data = access_allmodes_muni_m,
    aes(x = AREA_NA8, y = real_pop_park_ratio),
    inherit.aes = FALSE,
    color = "black",
    size = 3,
    shape = 17
  ) +
  labs(subtitle = "Total Constrained M", x = " ",
    y = "Multimodally Accessible Population per Parkland Hectre",
    fill = "Mode"
  ) +
  theme_minimal() +
  theme(
...
mm_S_muni_plot <- ggplot(access_allmodes_muni_m, aes(x = AREA_NA8, y = mm_opp, fill = mode)) +
  geom_bar(stat = "identity", position = position_dodge(width = 0.8)) +
  geom_point(
    data = access_allmodes_muni_m,
    aes(x = AREA_NA8, y = real_pop_park_ratio),
    ...
    size = 3,
    shape = 17) +
  labs(subtitle = "Singly Constrained M", x = " ",
    y = "Multimodally Accessible Population per Parkland Hectre",
    fill = "Mode") +
  theme_minimal() +
  theme(axis.text.x = element_text(angle = 45, hjust = 1))

mm_T_S_muni_plot <- mm_T_muni_plot / mm_S_muni_plot
ggsave("./figures/chp5-mm_mm_T_S_muni_plots.png", width = 10, height = 9, dpi = 300)
```
...
access_allmodes_muni_m <- access_allmodes_muni_m |> group_by(AREA_NA8, mode) |>
  summarise(V_opp = sum(V_opp_i_m),
            V_tot = sum(V_tot_i_m),
            prop_popm = sum(pop_m),
...
access_allmodes_muni_m <- access_allmodes_muni_m |>
  mutate(x_pos = as.numeric(factor(AREA_NA8, levels = neighbourhood_levels))) |>
  mutate(vv_opp = V_opp*10000/prop_popm,
         vv_tot = V_tot*10000/prop_popm)
access_allmodes_muni_m$mode <- factor(access_allmodes_muni_m$mode)

access_allmodes_muni_m <- access_allmodes_muni_m |> left_join(real_total_parks |> select(-c("x_pos")), by = "AREA_NA8")

access_allmodes_muni_m <- access_allmodes_muni_m |>
  mutate(real_park_pop_ratio = real_parkland_ha*10000/real_pop)
```

Plots of the vv_opp and vv_tot by muni, plus 12sqm/person threshold
```{r, eval=FALSE}
vv_T_muni_plot <- ggplot(access_allmodes_muni_m, aes(x = AREA_NA8, y = vv_tot, fill = mode)) +
  geom_bar(stat = "identity", position = position_dodge(width = 0.8)) +
  geom_hline(yintercept = 12, color = "red", linetype = "dashed", size = 1) +
    geom_point(
    data = access_allmodes_muni_m,
    aes(x = AREA_NA8, y = real_park_pop_ratio),
    inherit.aes = FALSE,
    color = "blue",
    size = 3,
  ...
  labs(subtitle = "Total Constrained V", x = " ",
    y = "Multimodally Accessible Parkland per Capita",
    fill = "Mode"
  ) +
  theme_minimal() +
  theme(
...
vv_S_muni_plot <- ggplot(access_allmodes_muni_m, aes(x = AREA_NA8, y = vv_opp, fill = mode)) +
  geom_bar(stat = "identity", position = position_dodge(width = 0.8)) +
  geom_hline(yintercept = 12, color = "red", linetype = "dashed", size = 1) +
      geom_point(
    data = access_allmodes_muni_m,
  ...
  coord_cartesian(ylim = c(0, 80)) +
  labs(subtitle = "Singly Constrained V", x = " ",
    y = "Multimodally Accessible Parkland per Capita",
    fill = "Mode"
  ) +
  theme_minimal() +
...
vv_T_S_muni_plot <- vv_T_muni_plot / vv_S_muni_plot
ggsave("./figures/chp5-mm_vv_T_S_muni_plots.png", width = 10, height = 9, dpi = 300)
```



Plots of the V_opp and V_tot by muni and pops..
```{r, eval=FALSE}
V_T_muni_vs_real_parkland_plot <- ggplot() +
  ...
  scale_fill_manual(values = Set2, name = "Parkland (ha)\naccessible by:") +

  scale_y_continuous(name = "Total Population",
                     limits = c(0, 1000000),
                     sec.axis = sec_axis(~ . / 200, name = "Total Multimodally Accessible Parkland Area (ha)"))+ #Mode of\nAccessible\nPopulation
...
V_S_muni_vs_real_parkland_plot <- ggplot() +
  ...
  scale_fill_manual(values = greys2, name = "Modal Population:") +
  new_scale_fill() +
  
  geom_col(data = access_allmodes_muni_m, aes(x = x_pos  +0.2, y = V_opp*200, fill = mode), width = 0.4, position = "stack") +
  scale_fill_manual(values = Set2, name = "Parkland (ha)\n accessible by:") +

  scale_y_continuous(name = "Total Population",
                     limits = c(0, 1000000),
                     sec.axis = sec_axis(~ . / 200, name = "Total Multimodally Accessible Parkland Area (ha)"))+ 
  geom_point(data = real_total_parks, aes(x = x_pos+ 0.2, y = real_parkland_ha*200),
             color = "black", size = 2, shape = 16, alpha=0.7) +
  theme_minimal()+
  scale_x_continuous(breaks = 1:length(neighbourhood_levels), labels = neighbourhood_levels, name = NULL) +
...
## XX. Scatter plot of uni modal walk vs multimodal walk-specific
...
```{r}
load(file="./intermediate/access_WALK_i_df.rda")
load(file="./intermediate/access_allmodes_i_m_df.rda")

access_WALK_i_df <- access_WALK_i_df |>
  mutate(vv_tot_i = ifelse(V_tot_i == 0 | Population == 0, 0, V_tot_i*10000/Population),
           vv_opp_i = ifelse(V_opp_i == 0 | Population == 0, 0, V_opp_i*10000/Population))

access_allmodes_i_m_df <- access_allmodes_i_m_df |>
  mutate(pop_m= replace_na(pop_m, 0),
         vv_tot_i_m = ifelse(V_tot_i_m == 0 | pop_m == 0, 0, V_tot_i_m*10000/pop_m),
         vv_opp_i_m = ifelse(V_opp_i_m == 0 | pop_m == 0, 0, V_opp_i_m*10000/pop_m))

access_allmodes_i_m_df_WALK <- access_allmodes_i_m_df |> as.data.frame() |>
  filter(mode == "Walk")  
```

```{r}
comparison_df <- left_join(access_WALK_i_df |>
                             select(DB_UID, vv_tot_i, vv_opp_i, Population),
                             access_allmodes_i_m_df |> as.data.frame() |> 
                             filter(mode == "Walk") |>
                             select(DB_UID, vv_tot_i_m, vv_opp_i_m, pop_m), by = "DB_UID")
...
```{r, eval=FALSE}
model_tot  <- lm(vv_tot_i_m ~ vv_tot_i, data = comparison_df)
model_tot  |> summary()

total_const_WALK_parkland_comparison_scatter <- 
  ggplot(comparison_df |> filter(vv_tot_i > 0 & vv_tot_i_m > 0  ), aes(x = vv_tot_i, y = vv_tot_i_m)) +
  geom_abline(slope = coef(model_tot)[["vv_tot_i"]],
...
```
```{r}
model_opp <- lm(vv_opp_i_m ~ vv_opp_i, data = comparison_df)
model_opp |> summary()

singly_const_WALK_parkland_comparison_scatter <- 
  ggplot(comparison_df |> filter(vv_opp_i > 0 & vv_opp_i_m > 0  ), aes(x = vv_opp_i, y = vv_opp_i_m)) +
  geom_abline(slope = coef(model_opp)[["vv_opp_i"]],
...
access_allmodes_i_m_parksize <- access_allmodes_i_m_parksize |>
  left_join(TO_census_21_DB |> st_drop_geometry() |> select(c("DB_UID","N_name")),
            by=c("from_id"="DB_UID"))
