...
```{r}
access_WALK |> group_by(P_ID) |> summarize(park_a = first(area_ha)) |> pull(park_a) |> sum()
```

## 5. calc singly constrained V and M
```{r}
silly <- access_WALK |> filter(POP2021 !=0) |> 
  left_join(B_j_WALK,
            by = "P_ID") |>
...
#checks 
silly$V_opp_ij |> sum() #should be 7632.922 ha total
