...
set_cancensus_api_key("PUT IN YOUR OWN KEY", overwrite=TRUE) 
```

# Origin import
Methodology for origin workflow (retrieved from city staff doc):
...
Checks:
```{r}
parkland_entrance_points$P_ID |> unique() |> length() # 1204 parks have paths out of 1607 parks, so #403 are missing entrance points
parkland_entrance_points$P_piece_ID |> unique() |> length() # 1408 park pieces have paths out of 1958 park pieces, so #550 are missing entrance points
```
...
potentialentrance_points_by_P_piece |>
  dplyr::select(c("P_piece_ID","P_ID","park_name", "area_m2", "geometry")) |>
  mutate(entrance_type = "Geometric centroid") |>
  mutate(P_piece_ent_ID = seq(5725,5724+nrow(potentialentrance_points_by_P_piece)))
)
```

Checking the parks add up:
```{r}
parkland_edge_and_centroid_entrance_points$P_ID |> unique() |> length() #1607 parks overall,
