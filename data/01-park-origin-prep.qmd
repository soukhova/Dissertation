...
set_cancensus_api_key("CensusMapper_53e5295237d9965a06425f1138ee6db7", overwrite=TRUE) #REPLACE WITH YOUR OWN CANCENSUS ID
```
...
```{r, eval=FALSE}
leaflet(st_transform(parkland, crs = 4326)) |>
  addTiles() |>
  addPolygons(
    fillColor = "black",
    weight = 1,
...
```{r}
parkland_entrance_points_count_by_P_piece <- parkland_entrance_points |>
  group_by(P_piece_ID) |>
  summarise(count_entrances_P_piece_ID = n())

parkland_entrance_points_count_by_P <- parkland_entrance_points |>
  group_by(P_ID) |>
...
```{r}
parkland_entrance_points_count_by_P$P_ID |> unique() |> length() # should be 1607, so #403 are missing entrance points (or 1204 are there)
parkland_entrance_points_count_by_P_piece$P_piece_ID |> unique() |> length() # should be 1958 , so #550 are missing entrance points (or 1408 are there)
```

Median of 3, but some have a max of 68! What parks have over 5 (Q3)? It's probably the larger parks that have more entrances. Let's test this out. Also, let's add the City of Toronto's Parkland strategy classification to the parks. 
```{r}
parkland <- full_join(parkland, parkland_entrance_points_count_by_P |> st_drop_geometry(), by="P_ID") |>
  full_join(parkland_entrance_points_count_by_P_piece |> st_drop_geometry(), by="P_piece_ID")
```
...
ggplot(parkland_grouped |> st_drop_geometry() |> filter(area_ha <= 50),
       aes(x = area_ha, y = count_entrances, col = park_size)) +
  geom_point() +          
...
Anyways. To err on the safer (and lazier) side, I'll just keep all the entrances.


But,  we need to add entrances to parks that have none. First, what are the parks that don't have entrances? Its 403 parks (out of 1607) or 550 pieces of park (out of 1958)! That's a lot, about ~25% of parks don't have some 'path'. 
```{r}
...
```{r}
parkland_pieces_noentrances <- parkland |> filter(!is.na(count_entrances_P_piece_ID))

potentialentrance_points <- parkland_pieces_noentrances |> 
...
Checks:
```{r}
potentialentrance_points$P_ID |> unique() |> length() # should be 1607 total parks- 403 parks missing entr, or 1204 unique park missing entrances
potentialentrance_points$P_piece_ID |> unique() |> length() # should be 1958 total park pieces- 550 park pieces missing entr, or 1408 unique park pieces with entrances
```
...
leaflet() %>%
  ...
  addPolygons(data = parkland_pieces_noentrances |> st_transform(4326),
              fillColor = "black",
              fillOpacity = 0.4,
              color = "white",
              weight = 1,
  ...
  addCircleMarkers(data = potentialentrance_points  |> st_transform(4326),
                   radius = 4,
                   color = "red",
                   fill = TRUE,
                   fillOpacity = 0.9,
...
parkland_edge_and_centroid_entrance_points <- rbind(
  parkland_entrance_points |> 
    mutate(entrance_type = "Edge intersection")  #the actual edge intersection entrances
  ,
  potentialentrance_points |>
    select(-c(area_ha, count_entrances_P_piece_ID, count_entrances_P_ID)) |>
    mutate(P_piece_ent_ID = seq(5725,5724+nrow(potentialentrance_points)),
         entrance_type = "Geometric centroid"))  #the potential ones
save(parkland_edge_and_centroid_entrance_points, file="./parkland_edge_and_centroid_entrance_points.rda")
```

Checks:
```{r}
parkland_edge_and_centroid_entrance_points$P_ID |> unique() |> length() # should be 1607 total parks- 403 parks missing entr, or 1204 unique park missing entrances
parkland_edge_and_centroid_entrance_points$P_piece_ID |> unique() |> length() # should be 1958 total park pieces- 550 park pieces missing entr, or 1408 unique park pieces with entrances
```
